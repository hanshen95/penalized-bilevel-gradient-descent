...
parser.add_argument('--lrx', type=float, default=0.3)
parser.add_argument('--lry', type=float, default=0.5)
parser.add_argument('--gamma_init', type=float, default=0)
parser.add_argument('--gamma_max', type=float, default=37)
parser.add_argument('--gamma_argmax_step', type=int, default=5000)
parser.add_argument('--outer_itr', type=int, default=40000)
...
# initialize recorder
log_path =('./save/gpbg_clean{}_trsize{}_pollute{}_net{}_lr{}_'+
         'gaminit{}max{}step{}_outeritr{}_reg{}_seed{}.csv').format(args.dataset,
          len(tr.data),tr.rho,args.net,(args.lrx,args.lry),args.gamma_init,args.gamma_max,
          args.gamma_argmax_step, args.outer_itr,args.reg,seed)
f = open(log_path, "w+")
f.close()
with open(log_path, 'a', encoding='utf-8') as f:
    csv_writer = csv.writer(f)
    csv_writer.writerow([args])
    csv_writer.writerow(['step','acc','p','F1 score','f','|dgdy|','f+gam/2|dgdy|^2','itr time'])
...
for k in range(args.outer_itr):  
    ...
    loss = lr_decay*(fy + gam/2* loss_F(dgdy))
    loss.backward()
    x_opt.step()
    y_opt.step()
    gam+= step_gam
    ...
    if k%10 == 0:
        ...
        with torch.no_grad():
            ...
            # report and record
            dgdy_norm = loss_F(dgdy).pow(0.5)
            F_ = loss/lr_decay
            print('step={},acc={:.3f},p={:.3f},F1 score={:.3f},f={:.3f},|dgdy|={:.3f},F={:.3f},itrtime={:.6f}'.format(k,acc,p,score,fy,dgdy_norm,F_,itr_time))
            with open(log_path, 'a', encoding='utf-8', newline='') as f:
                csv_writer = csv.writer(f)
                csv_writer.writerow([k,float(acc),float(p),float(score),float(fy),float(dgdy_norm),float(F_),itr_time])
